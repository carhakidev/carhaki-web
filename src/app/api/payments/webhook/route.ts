...
export async function POST(req: NextRequest) {
  try {
    ...
    if (event.event === 'charge.success') {
      ...
      if (!reference || !userId || !vin) return NextResponse.json({ received: true });

      const existing = await prisma.order.findUnique({ where: { paystackReference: reference } });
      if (existing && existing.paymentStatus.toString() === 'SUCCESS') {
      ...
      if (!existingReport) {
        const report = await prisma.report.create({
          data: { orderId: order.id, userId, vin, status: 'PENDING' as never },
        });
